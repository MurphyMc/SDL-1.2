...
static SDLKey VNC_TranslateKeycode(rfbKeySym kc);


void VNC_on_key (rfbBool down, rfbKeySym key, rfbClientPtr cl)
{
  struct SDL_PrivateVideoData * self = ((SDL_VideoDevice*)cl->screen->screenData)->hidden;
  SDL_keysym keysym = {0};
  keysym.scancode = key;
  keysym.sym = VNC_TranslateKeycode(key);
  keysym.mod = self->key_mod; // KMOD_NONE

  int mod = 0;
  switch (keysym.sym)
  {
    case SDLK_NUMLOCK: mod = KMOD_NUM; break;
    case SDLK_CAPSLOCK: mod = KMOD_CAPS; break;
    ...
    case SDLK_LMETA: mod = KMOD_LMETA; break;
    case SDLK_RMETA: mod = KMOD_RMETA; break;
    default: break;
  }
  if (mod)
  {
  ...
  keysym.unicode = 0;
  //FIXME: Unicode conversion is *very* bad.
  if ( SDL_TranslateUNICODE && keysym.sym <= 127)
  {
    char c = keysym.sym;
    if (keysym.mod & KMOD_CTRL)
    {
      if (c >= 96 && c <= 127)
        c -= 96; // It was "lowercase"
      else if (c >= 64 && c <= 95)
        c -= 64;
      else
        c = 0;
...
#ifdef DEBUG_KEYS
  fprintf(stderr, "Key %s %08x -> %04x\n",down?"DOWN":"UP  ",key,keysym.sym);
#endif
...
static SDLKey VNC_TranslateKeycode(rfbKeySym xsym)
{
